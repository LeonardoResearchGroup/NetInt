...
public class VNodeDescription implements Serializable {
	...
	public void show(VNode vNode) {
		...
		if (vNode.isMouseOver) {
			...
			while (itr.hasNext()) {
				int tier = itr.next();
				shift = count * 35;
				Canvas.app.text("Com: " + vNode.getNode().getCommunity(tier), vNode.getPos().x + 5, vNode.getPos().y - 60 - shift);
				Canvas.app.text("in: " + vNode.getNode().getInDegree(tier), vNode.getPos().x + 5, vNode.getPos().y - 50 - shift);
				Canvas.app.text("out: " + vNode.getNode().getOutDegree(tier), vNode.getPos().x + 5, vNode.getPos().y - 40 - shift);
				count++;
			}
	...
	public void show(VCommunity vComm) {
		...
		if (vComm.isMouseOver) {
			...
			// canvas.app.fill(setColor(50, 150));
			Canvas.app.fill(new Color(50, 50, 50, 150).getRGB());
			Canvas.app.rect(vComm.getPos().x - 5, vComm.getPos().y - 3, width, height - heightComplement);
			// canvas.app.fill(setColor(200, 170));
			Canvas.app.fill(new Color(200, 200, 200, 170).getRGB());
			...
			// Communities data
			Iterator<Integer> itr = vComm.getNode().getMetadataKeys().iterator();
			int count = 0;
			int shift = 0;
			while (itr.hasNext()) {
				int tier = itr.next();
				shift = count * 35;
				Canvas.app.text("Com: " + vComm.getNode().getCommunity(tier), vComm.getPos().x + 5, vComm.getPos().y - 60 - shift);
				Canvas.app.text("in: " + vComm.getNode().getInDegree(tier), vComm.getPos().x + 5, vComm.getPos().y - 50 - shift);
				Canvas.app.text("out: " + vComm.getNode().getOutDegree(tier), vComm.getPos().x + 5, vComm.getPos().y - 40 - shift);
				count++;
			}

			Canvas.app.strokeWeight(1);
			Canvas.app.line(vComm.getPos().x + 5, vComm.getPos().y - 80 - shift, vComm.getPos().x + 150, vComm.getPos().y - 80 - shift);
