...
public class Filters {
	...
	public void setGraph(Graph<Node, Edge> rootGraph) {
		...
		System.out.println(
				getInstance().getClass().getName() + " Total edges in Filters: " + remainingGraph.getEdgeCount());
	}
	...
	public static DirectedSparseMultigraph<Node, Edge> filterByCommunity(
			Graph<Node, graphElements.Edge> jungGraph, final String community) {
	...
	public static DirectedSparseMultigraph<Node, Edge> filterByInterCommunities(
			Graph<Node, graphElements.Edge> jungGraph, final String communityNameA,
			final String communityNameB) {
		// Filter and extractor
		EdgePredicateFilter<Node, Edge> filter = new EdgePredicateFilter<Node, Edge>(
				Predicates.edgeLinkingGraphs(communityNameA, communityNameB));
		DirectedSparseMultigraph<Node, Edge> problemGraph = (DirectedSparseMultigraph<Node, Edge>) filter
				.transform(jungGraph);
		return problemGraph;
	...
	/**
	 * Returns a subgraph of jungGraph whose edges connect the specified
	 * communities in either direction AND REMOVES edges from the source
	 * jungGraph
	 * 
	 * @param jungGraph
	 * @param comumnity1
	...
	public static DirectedSparseMultigraph<Node, Edge> filterAndRemoveCommunityLinks(final String communityNameA,
			final String communityNameB) {
		// Filter and extractor
		EdgePredicateFilterAndExtractor<Node, Edge> filter = new EdgePredicateFilterAndExtractor<Node, Edge>(
				Predicates.edgeLinkingGraphs(communityNameA, communityNameB));
		DirectedSparseMultigraph<Node, Edge> problemGraph = (DirectedSparseMultigraph<Node, Edge>) filter.transform(remainingGraph);
		System.out.println(
				getInstance().getClass().getName() + " Remaining edges in Filters: " + remainingGraph.getEdgeCount());
		return problemGraph;
